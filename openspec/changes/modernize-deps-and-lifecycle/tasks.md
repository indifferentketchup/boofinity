...
- [x] 1.1 Apply the upstream hf_hub fix: from the repo root run `git show 2ecb218 | git apply`.
...
- [x] 1.2 Trim `transformer/acceleration.py` to the TF32 block only (current lines 16-22):
...
- [x] 1.3 Add `from infinity_emb.transformer import acceleration  # noqa: F401` (TF32 side
...
- [x] 1.4 Remove BT call sites in `transformer/embedder/sentence_transformer.py`
...
- [x] 1.5 Remove BT call sites in `transformer/crossencoder/torch.py` (32-33, 48-49, 73-74)
...
- [x] 1.6 Clean up commented-out BT references in `transformer/audio/torch.py` (line 34)
...
- [x] 1.7 Flip the env default: `env.py:147` `default=["true"]` to `default=["false"]`.
...
- [x] 1.8 Lift pins in `libs/infinity_emb/pyproject.toml`: line 25 uvicorn `^0.32.0` to
...
- [x] 1.9 Build `.venv-batch2` with the BASELINE_NOTES.md recipe (venv, pip upgrade, CPU
...
- [x] 1.10 Co-install acceptance for BT removal: `.venv-batch2/bin/pip install optimum
...
- [x] 1.11 Parity gate: `cd tests/parity && ../../.venv-batch2/bin/python check_parity.py`
...
- [x] 2.1 In `inference/batch_handler.py`: add `if self._shutdown.is_set(): break` after a
...
- [x] 2.2 Add `INFINITY_REQUEST_TIMEOUT_S` to `env.py` via the MANAGER pattern (default
      `0`, disabled). Verify: `MANAGER.request_timeout_s` resolves with default and
      with the env var set.
- [x] 2.3 Wrap the result gather (`batch_handler.py:329-331`) in `asyncio.wait_for` when
...
- [x] 2.4 Gate `/health` (`infinity_server.py:162-170`): resolve the array defensively,
...
- [x] 2.5 New unit test: gather timeout. With debugengine and a monkeypatched
      ...
      behavior holds.
- [x] 2.6 New tests: shutdown responsiveness and health gate. (a) debugengine with items
...
- [x] 2.7 Final gate: re-run `cd tests/parity && ../../.venv-batch2/bin/python
