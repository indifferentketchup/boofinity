...
import boofinity
from boofinity._optional_imports import CHECK_TYPER, CHECK_UVICORN
from boofinity.args import EngineArgs
from boofinity.env import MANAGER
from boofinity.log_handler import UVICORN_LOG_LEVELS, logger
...
from boofinity.infinity_server import create_server
...
if CHECK_TYPER.is_available:
    ...
    def v1(
        ...
        """Infinity API ♾️  cli v1 - deprecated, consider use cli v2 via `boofinity v2`."""
        if api_key:
    ...
    if len(sys.argv) == 1 or sys.argv[1] not in [
        ...
        if len(sys.argv) == 1 or sys.argv[1] not in ["v1", "v2", "help", "--help"]:
            logger.error(
                ...
                f"This is deprecated since 0.0.32. You are on {boofinity.__version__}. "
                "Usage: `boofinity v2 --model-id BAAI/bge-large-en-v1.5. "
