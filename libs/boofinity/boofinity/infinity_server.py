...
from boofinity.engine import AsyncEmbeddingEngine, AsyncEngineArray
from boofinity.env import MANAGER
from boofinity.fastapi_schemas import docs, errors
...
from boofinity.telemetry import PostHog, StartupTelemetry, telemetry_log_info

if TYPE_CHECKING:
    from boofinity.fastapi_schemas.pymodels import DataURIorURL
...
def create_server(
    *,
    ...
    async def _embeddings(data: MultiModalOpenAIEmbedding):
        ...
        ```bash
        boofinity v2 --model-id BAAI/bge-small-en-v1.5 --model-id openai/clip-vit-base-patch32 --model-id laion/larger_clap_general
        ```
        """
